...
async function guard(){
  ...
  const {data:ent,error}=await supabase.from('hub_service_access')
    .select('service_code,service_label,status,metadata')
    .eq('user_id',session.user.id).eq('service_code','mia').eq('status','active').maybeSingle();
  if(error||!ent){ await supabase.auth.signOut(); location.replace('./login.html?access=missing'); return; }
